...
선택적 실제 Codex Smoke Test는 `npm run test:integration:codex`로 실행하며 일반 테스트에는 실제 계정 호출이 없습니다. Windows 빌드는 `release/LG-Report-Agent-1.0.0-x64-Portable.exe`와 `release/LG-Report-Agent-Setup-1.0.0-x64.exe`를 생성합니다. 코드 서명이 구성되지 않았으므로 산출물은 unsigned이며 SmartScreen 경고가 나타날 수 있습니다.
